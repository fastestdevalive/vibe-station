...
> - **If you see agy running into a usage/rate limit mid-dispatch** (the terminal shows a limit/quota message), switch the model **within that same running session** using agy's own in-terminal `/model` slash command rather than terminating and redispatching. **The exact model id is `gpt-oss-120b-medium`** (a bare `/model gpt oss 120b` is rejected as invalid — send `/model gpt-oss-120b-medium` via `vst session send <id> "/model gpt-oss-120b-medium" --no-wait`, verify it lands per the delivery-hiccup note above). **If that model itself returns a transient `503 UNAVAILABLE (no capacity available)`** (a real, observed failure mode, distinct from the quota error and not fixed by retrying the same model), fall further to another model already listed in that session's own `/model` output — `claude-sonnet-4-6` worked when this happened. The full list agy exposes as of this session: `gemini-3.8-flash-{high,medium,low}`, `gemini-3.7-flash-{high,medium,low}`, `gemini-3.6-flash-{high,medium,low}`, `gemini-3.1-pro-{high,low}`, `claude-sonnet-4-6`, `claude-opus-4-6-thinking`, `gpt-oss-120b-medium` — re-check this list from the session's own error output rather than assuming it's stable across time. This is a live fallback for hitting agy's own limit/capacity, distinct from the DeepSeek→Haiku escalation ladder in step D, and distinct from model-swap exception #2 itself.
...
> **One-off temporary model-swap #3 (2026-09-15e, user-directed — `09` only, NOT a reversal of model-swap exception #2 for `08`/`10`):** after the entire agy fallback chain (`agy-medium` → `gpt-oss-120b-medium` → `claude-sonnet-4-6` → `claude-opus-4-6-thinking`) hit the same account-wide quota exhaustion window (~4h47-51m reset) mid-dispatch on both `08` and `09` dispatch #3 simultaneously, the user directed finishing `09-cli-port` completely on `$DEEPSEEK_MODE` instead, while `08` stays parked on its stuck agy session (or is separately revisited) until agy's quota resets or the user says otherwise. **This does not change the default for `08`/`10` — they remain agy-medium per exception #2** unless a fresh, equally explicit instruction says otherwise. When picking a stuck agy session back up on a different model, the correct move is to **terminate the stuck session without discarding its uncommitted work** (the files stay on disk in the shared worktree) and dispatch a **fresh** implementer session with explicit instructions to review and finish that exact on-disk state — never re-port from scratch, and never let the new session touch another still-in-progress session's uncommitted files in a different crate. Also: **do not run the full `rust-gate.sh` while a sibling crate has uncommitted work** — its `cargo fmt --all --check` is workspace-wide regardless of the crate argument (see the fmt-false-positive note in `07a-3`'s/`09-2`'s closeout history) and will show unrelated diffs; use crate-scoped `cargo fmt -p <crate> --check` / `cargo clippy -p <crate> ...` / `cargo test -p <crate>` instead until the sibling's work is committed.
