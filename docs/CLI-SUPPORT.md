...
| **Agy** | Not supported | openab `agy-acp` (vendored submodule) exposes `session/new`, `session/load`, `session/prompt`, `session/cancel`, `session/setConfigOption`; no `_session/steering` method | Cancel-and-resend is the ceiling. `supportsSteering` will be `false` from the handshake; `submit()` falls through to `enqueue()`. |
