...
// ─── Phase 2 tests: groupEvents child correlation ────────────────────────────

function taskUseEvent(id: string, turnId: string): NormalizedEvent {
  return { id, sessionId: "s1", ts: "", provider: "claude", kind: "tool_use", toolName: "task", toolId: id, turnId };
}

// groupEvents produces "tool" items; mergeToolRuns converts them to "toolRun".
// We test the "tool" items directly from groupEvents since childSessionId is
// set during groupEvents, not mergeToolRuns.
describe("groupEvents — 2.T1: task tool_use with matching childByParent entry sets childSessionId", () => {
  it("assigns childSessionId from the FIFO list when parentSessionId matches", () => {
    const childByParent = new Map([["parent-sess", ["child-1"]]]);
    const items = groupEvents([taskUseEvent("tu-1", "t1")], "parent-sess", childByParent);
    const toolItem = items.find((i) => i.type === "tool");
    expect(toolItem).toBeTruthy();
    if (toolItem?.type === "tool") {
      expect(toolItem.childSessionId).toBe("child-1");
    }
  });
});

describe("groupEvents — 2.T2: task tool_use with empty map: childSessionId is undefined, no crash", () => {
  it("leaves childSessionId undefined when no child is queued", () => {
    const items = groupEvents([taskUseEvent("tu-1", "t1")], "parent-sess", new Map());
    const toolItem = items.find((i) => i.type === "tool");
    if (toolItem?.type === "tool") {
      expect(toolItem.childSessionId).toBeUndefined();
    }
  });

  it("works with no optional params (1-arg call-site compat)", () => {
    expect(() => groupEvents([taskUseEvent("tu-1", "t1")])).not.toThrow();
  });
});

describe("groupEvents — 2.T3: two task tool_use entries, two children in FIFO order", () => {
  it("assigns children to task entries in arrival order", () => {
    const childByParent = new Map([["parent-sess", ["child-1", "child-2"]]]);
    const events: NormalizedEvent[] = [taskUseEvent("tu-1", "t1"), taskUseEvent("tu-2", "t1")];
    const items = groupEvents(events, "parent-sess", childByParent);
    const toolItems = items.filter((i) => i.type === "tool");
    expect(toolItems).toHaveLength(2);
    if (toolItems[0]?.type === "tool" && toolItems[1]?.type === "tool") {
      expect(toolItems[0].childSessionId).toBe("child-1");
      expect(toolItems[1].childSessionId).toBe("child-2");
    }
  });
});
