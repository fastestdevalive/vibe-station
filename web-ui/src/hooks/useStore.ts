  ...
  persist(
      ...
      return {
        ...
        setActiveFile: (path) =>
          set((s) => {
            ...
            const tabs = s.openFileTabsByWorktree[key] ?? [];
            const idx = s.activeFileTabIdxByWorktree[key] ?? -1;
            // Already open in some tab → switch to it instead of writing the
            // same path into a second tab. Tab paths MUST stay unique: the tab
            // strip keys/identifies tabs by path, so a duplicate renders as a
    ...
    },
    {
      ...
      migrate: (persisted, version) => {
        ...
        // v17 → v18: drop duplicate paths already persisted into the tab
        // arrays (a pre-fix replace-active-tab could write a path that was
        // open elsewhere, producing dead "ghost" tabs), keeping the active
        // tab pointing at the same file.
        if (version < 18) {
          const tabs = (p.openFileTabsByWorktree as Record<string, string[]> | undefined) ?? {};
          const idxMap = (p.activeFileTabIdxByWorktree as Record<string, number> | undefined) ?? {};
          for (const [key, list] of Object.entries(tabs)) {
            ...
            if (deduped.length === list.length) continue;
            const storedIdx = idxMap[key] ?? -1;
            tabs[key] = deduped;
            if (storedIdx < 0) continue; // was deliberately deactivated — preserve that
            const activePath = list[storedIdx];
            const nextIdx = activePath != null ? deduped.indexOf(activePath) : -1;
            idxMap[key] = nextIdx >= 0 ? nextIdx : deduped.length > 0 ? 0 : -1;
          }
          p.openFileTabsByWorktree = tabs;
          p.activeFileTabIdxByWorktree = idxMap;
        }
